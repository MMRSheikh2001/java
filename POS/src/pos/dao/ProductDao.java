...
public class ProductDao implements DaoService<Product> {
    ...
    public void save(Product e) {
        sql = "insert into product (name,price,quantity,supplierId,categoryName) values(?,?,?,?,?)";
        try {
            ...
            ps.setString(5, e.getCategoryName());
            ps.executeUpdate();
            ps.close();
            db.getCon().close();
    ...
    public List<Product> findAll() {
        ...
        sql = "select p.id,p.name,p.price,p.quantity,s.name,p.categoryName from product p join supplier s on s.id=p.supplierId";
        try {
            ps = db.getCon().prepareStatement(sql);
            rs = ps.executeQuery();
            ...
            while (rs.next()) {
                ...
                Product p = new Product(rs.getInt("p.id"), rs.getString("p.name"),
                        rs.getDouble("p.price"),
                        rs.getDouble("p.quantity"),
                        rs.getString("s.name"), rs.getString("p.categoryName"));
                pList.add(p);
            }
            rs.close();
            ps.close();
    ...
    public void update(Product e) {
        sql = "update product set name =?,price =?,quantity =?,supplierId=?,categoryName=? where id=?";
        try {
            ps = db.getCon().prepareStatement(sql);
            ps.setString(1, e.getName());
            ps.setDouble(2, e.getPrice());
